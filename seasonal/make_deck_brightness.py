...
OUT = os.path.join(HERE, "figs", "deck")
...
def panel(ax, year, flag=None):
    ...
    ax.axis("off")
    ax.set_title(str(year), color=INK, fontsize=20, fontweight="700", pad=8)
    if flag:
...
def build(years, flags, fname, title, sub):
    ...
    cb.ax.tick_params(length=0, labelsize=11, colors=MUTED)
    cb.set_label("brighter →", color=INK_2, fontsize=12)
    pass
    pass
